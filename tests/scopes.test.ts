 ...
 *
 * This walk runs over a server with nothing behind it, with no key, a key akou does not know (what
 * a revoked key gets) and the app's token, which holds the `admin` scope; the `jobs` column is
 * also checked against the served `?scope=jobs` view. The same table is walked with real `jobs`,
 * `admin` and revoked keys in `server-access.e2e.test.ts` (SV-K3).
 */
...
/** The one table, shared with the walk with real keys in `server-access.e2e.test.ts`. */
const TABLE = ACCESS;
