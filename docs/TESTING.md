...
| # | Item | P | From | Acceptance | Today |
|---|---|---|---|---|---|
| TS-14 | A WebKit leg | P1 | the app ships the native webview: WKWebView, WebKitGTK, WebView2 | The UI suite also runs on Playwright's WebKit build on Linux, the closest stand-in for WebKitGTK and WKWebView. Both legs share the floor rules of TS-2. It stands in for the packaged webview until the Xvfb smoke of CI-22 exists | partial: `AKOU_UI_BROWSER=webkit` runs the suite in WebKit, as the `ui-webkit` job of `nightly.yml` with its own floor (`ui-webkit:linux`). On Linux WebKit 44 of 52 pass (Chromium 52 of 52, same container). The 8 failures, which the window work owns: the two W12.2 copy tests grant `clipboard-write`, a permission WebKit does not have; after Pause, Stop leaves the page on "stopping" for about 15 s on WebKit (54 ms on Chromium) though the call ended at once, which fails the Record/Pause/Stop, States and Share-after-Record tests; the share viewer never counts its viewer; both [W6.2] notepad tests lose the edit. The leg joins `ci.yml`'s `ui` matrix, under `ci-ok`, once it is green |
