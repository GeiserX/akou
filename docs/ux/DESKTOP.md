...
| Transcript text, answers or an ask box in the floating indicator | It stays on top while the user shares their screen | A click on it brings the window forward; the palette turns text into a question (`Mod+K`, "Ask: …", WINDOW W14.5) |
...
1. **The indicator.** A recording dot, the elapsed time, two thin level bars (you, the call), Mute and Stop. The elapsed time is the call's recorded time: it carries on across a new part and stands still while paused. A degraded capture turns the dot into the warning mark with one word ("call side silent"). Clicking the body shows the main window on the call; asking goes through the palette ([PRINCIPLES.md](PRINCIPLES.md)). The main process sends it only the live call's id, state and mute, the part, pause, resume and health events rebuilt field by field, and the levels; its only controls are stop, mute and unmute of the live call.
...
| Id | Feature | P | From | Acceptance | Today |
|---|---|---|---|---|---|
| DK-F1 | The floating indicator: dot, elapsed time, level bars, Mute, Stop, click to open; draggable, position remembered; hidden while the main window is focused; `app.floatingIndicator` on by default | P1 | Granola floating nub; Wispr Flow bar; MacWhisper overlay; intent: reach the live call during the call | UI test against the indicator page with a fake call: Stop stops, Mute mutes, a click opens the main window, and the dot changes on a `health {state: dead}` event. A content test renders the page with segments, a title and names carrying unique markers and finds none of them; a positive control injects one and fails. An RPC test finds none of the markers in anything sent to the indicator page, while the main window's RPC over the same call carries them (the positive control) Shell test: the indicator window is created on `call.created`, hidden on focus of the main window, closed on `part.ended` | has: `src/ui/indicator.ts` in its own ElectroBun view; UI tests drive it inside the real shell (`tests/ui/desktop.test.ts`), shell tests in `tests/desktop.test.ts`. The look over a real meeting and the drag are in the release checklist |
