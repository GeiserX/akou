...
Today there is one hotkey, `app.hotkey`, default `Option+Command+R` on macOS and `Control+Alt+R` elsewhere. It is registered once at start. A change waits for a restart and nothing says so. When registration fails, only the log hears about it.
