...
| SI-8 | `akou serve` in the darwin-arm64 CLI: starts the server in the foreground on macOS with the same settings as the image | P1 | A Mac is a likely transcription box, and Docker on macOS has no Metal (SV-R3) | On a macOS arm64 runner, `akou serve` answers `/healthz` and a submitted clip completes. SV-P8 covers Linux | missing |
