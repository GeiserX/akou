...
| `servers[0].url` is akou's public address | `https://akou.example` from `server.public_host`, else the request's `Host` | `addSpec` calls the spec's first server when no `baseUrl` is given |
