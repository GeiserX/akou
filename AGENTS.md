...
- `bun run check`: Biome, `tsc --noEmit`, then `bun test` and its floor. CI runs exactly this on macOS, Windows and Linux.
- Every CI test job has a floor in `tests/floors.json` (`scripts/ci/test-floor.ts`): fewer passes or more skips fail the job. Adding tests needs no edit; a new skip raises `maxSkip` in the same diff. A skip is only `test.skipIf(cond)` for a missing model, device or OS, with the reason in the test name; Biome fails `test.only` and `test.skip` under `tests/`.
